# END USER LICENSE AGREEMENT (EULA)
**For KREDIX XYPHER OPTIMIZER v5.0**  
*Last Updated: [DATE]*

## 1. DEFINITIONS
- "Software" = KREDIX XYPHER OPTIMIZER (all versions)
- "You" = Licensee/user
- "We" = [Your Name/Company], the licensor

## 2. LICENSE GRANT
We grant you a **non-exclusive, non-transferable, revocable** license to:
- Use the Software for personal/non-commercial purposes
- Install on up to 3 Windows devices you own

## 3. RESTRICTIONS
You **MAY NOT**:
- Reverse engineer, decompile, or disassemble the Software
- Redistribute, sell, or sublicense the Software
- Use in government/military systems without written permission
- Remove copyright notices

## 4. NO WARRANTY
**THE SOFTWARE IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND.**  
We expressly disclaim:
- Fitness for any particular purpose
- Non-infringement
- Continuous operation without errors
- Compatibility with all hardware/software

## 5. LIMITATION OF LIABILITY
**IN NO EVENT SHALL WE BE LIABLE FOR:**
- Data loss or corruption
- System instability or crashes
- Hardware damage
- Lost profits or business interruption
- Any direct/indirect/consequential damages

Maximum liability (if any) shall not exceed **$10 USD**.

## 6. USER RESPONSIBILITIES
You agree to:
1. Create system restore points before use
2. Backup all critical data
3. Not use in production environments without testing
4. Accept all risks of optimization

## 7. AUTOMATIC DATA COLLECTION
The Software **does not**:
- Collect personal files
- Transmit usage data (unless opt-in analytics enabled)
- Store IP addresses

## 8. TERMINATION
We may terminate this license if you:
- Violate any EULA terms
- Attempt to bypass security measures
- Use the Software illegally

## 9. GOVERNING LAW
This agreement shall be governed by the laws of **[Your Country/State]**, with exclusive jurisdiction in **[Your City] courts**.

## 10. ENTIRE AGREEMENT
This EULA constitutes the complete agreement between parties regarding the Software.

---

**BY USING THE SOFTWARE, YOU ACKNOWLEDGE THAT:**
1. You have read this EULA
2. You understand the risks
3. You accept all terms unconditionally

